...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        scanf("%d %d %d", &processes[i].burstTime, &processes[i].arrivalTime, &processes[i].priority);
...
// Function to reset waiting, turnaround, and remaining times for each scheduling choice
        ...
        for (int i = 0; i < n; i++) {
            if (processes[i].remainingTime > 0 && time >= processes[i].arrivalTime) {
                if (processes[i].remainingTime > quantum) {
                    ...
                    processes[i].waitTime = time - processes[i].burstTime - processes[i].arrivalTime;
