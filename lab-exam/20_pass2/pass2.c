...
void main()
{
	...
	// Process each line until we reach the END directive
	while (strcmp(opcode, "END") != 0)
	{
		flag = 0;  // Reset flag to indicate if opcode is found in opcode table

		// Read each entry in the opcode table to find a matching opcode
		...
		else if ((strcmp(opcode, "BYTE") == 0) || (strcmp(opcode, "WORD") == 0))
		{
			...
			strcpy(objectcode, "\0");               // Set object code to empty string if no match
